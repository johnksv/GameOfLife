...
public enum Tone {
    ...
    public double makeTone(Tone tone, long xPosition){
        return Math.sin(2.0*Math.PI*tone.frequency*xPosition);
    }
    
    public double makeAccord(Tone tone, long xPosition, Tone... supplement){
        double accord = makeTone(this, xPosition);
        for (Tone toneToAdd : supplement){
            accord += makeTone(toneToAdd, xPosition);
        }
        return accord;
    }
